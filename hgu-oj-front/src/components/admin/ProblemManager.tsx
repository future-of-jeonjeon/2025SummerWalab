            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div className="space-y-1">
                        ...
                        <p className="text-sm text-gray-500 dark:text-slate-400">등록된 문제를 관리합니다.</p>
                    </div>
                </div>
                ...
                <div className="overflow-hidden rounded-lg border border-gray-200 dark:border-slate-700 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50 dark:bg-slate-800">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase">난이도</th>
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase">상태</th>
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase">작성자</th>
                                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-slate-400 uppercase">관리</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-200 dark:divide-slate-700 bg-white dark:bg-slate-900">
                            {problemListLoading ? (
                                <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-gray-500 dark:text-slate-400">로딩 중...</td></tr>
                            ) : problemListError ? (
                                <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-red-600">{problemListError}</td></tr>
                            ) : problemList.length === 0 ? (
                                <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-gray-500 dark:text-slate-400">문제가 없습니다.</td></tr>
                            ) : (
                                problemList.map((problem) => (
                                    <tr key={problem.id} className="hover:bg-gray-50 dark:hover:bg-slate-800 dark:hover:bg-slate-800">
                                        <td className="px-4 py-3 text-sm text-gray-900 dark:text-slate-100">{problem.id}</td>
                                        <td className="px-4 py-3 text-sm text-gray-900 dark:text-slate-100 font-medium">{problem.title}</td>
                                        <td className="px-4 py-3 text-sm text-gray-500 dark:text-slate-400">{problem.difficulty}</td>
                                        <td className="px-4 py-3 text-sm">
                                            <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${problem.visible ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600 dark:text-slate-400'}`}>
                                                {problem.visible ? '공개' : '비공개'}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3 text-sm text-gray-500 dark:text-slate-400">{problem.createdBy?.realName || problem.createdBy?.username || '-'}</td>
                    ...
                    <div className="border-t border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 px-4 py-3 flex justify-between items-center">
                        <span className="text-sm text-gray-700 dark:text-slate-300">총 {problemTotal}개</span>
                        <div className="flex gap-2">
