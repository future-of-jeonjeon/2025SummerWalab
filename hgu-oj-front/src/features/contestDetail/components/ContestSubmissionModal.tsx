    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" onClick={onClose}>
      <div className="max-h-[90vh] w-full max-w-3xl overflow-hidden rounded-lg bg-white dark:bg-slate-900 shadow-xl" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-gray-200 dark:border-slate-700 px-5 py-3">
          ...
          <button type="button" onClick={onClose} className="rounded-md bg-gray-100 dark:bg-slate-800 px-3 py-1 text-sm font-medium text-gray-700 dark:text-slate-200 hover:bg-gray-200 dark:hover:bg-slate-700">
            닫기
          </button>
        </div>
        <div className="max-h-[70vh] overflow-y-auto px-5 py-4 text-sm text-gray-800 dark:text-slate-200">
          {loading ? (
            ...
            <div className="space-y-4">
              <div className="grid gap-2 text-gray-700 dark:text-slate-300 sm:grid-cols-2">
                <div>
              ...
              <div>
                <h4 className="mb-2 font-semibold text-gray-900 dark:text-slate-100">소스 코드</h4>
            ...
            <div className="text-sm text-gray-600 dark:text-slate-400">제출 정보를 불러오지 못했습니다.</div>
          )}
        </div>
      </div>
    </div>
