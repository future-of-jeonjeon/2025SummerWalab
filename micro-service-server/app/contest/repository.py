...
async def find_contest_by_id(
        contest_id: int,
        db: AsyncSession) -> Optional[Contest]:
    return await db.get(Contest, contest_id)


async def find_contest_language_by_contest_id(
        contest_id: int,
        db: AsyncSession) -> ContestLanguage:
    stmt = (
        select(ContestLanguage)
        .where(ContestLanguage.contest_id == contest_id)
    )
...
async def update_contest(
        contest: Contest,
        db: AsyncSession) -> Contest:
    await db.flush()
    await db.refresh(contest)
    return contest


async def create_contest_language(
...
async def update_contest_language(
        contest_language: ContestLanguage,
        languages: List[str],
        db: AsyncSession) -> ContestLanguage:
...
async def exists_display_id_in_contest(
        contest_id: int,
        display_id: str,
...
async def get_participated_contest_by_user_id(
        user_id: int,
        db: AsyncSession) -> List[Contest]:
    stmt = (
        select(Contest)
        .join(ContestUser, ContestUser.contest_id == Contest.id)
        .where(ContestUser.user_id == user_id)
    )
    result = await db.execute(stmt)
    return result.scalars().all()


async def get_all_contests_with_languages(
        db: AsyncSession):
    stmt = (
        select(Contest, ContestLanguage.languages, User, UserData)
        .join(User, Contest.created_by_id == User.id)
...
async def count_contest_participants_by_contest_id(
        contest_id,
        db: AsyncSession):
    stmt = (
        select(func.count()).select_from(ContestUser)
...
async def get_contest_detail_by_id(
        contest_id: int,
        db: AsyncSession):
    stmt = (
        select(Contest, ContestLanguage.languages, User, UserData)
...
async def delete_contest_languages(
        contest_id: int,
        db: AsyncSession) -> None:
    stmt = (
        delete(ContestLanguage)
        .where(ContestLanguage.contest_id == contest_id)
    )
    await db.execute(stmt)


async def delete_contest_users(
        contest_id: int,
        db: AsyncSession) -> None:
    stmt = (
        delete(ContestUser)
...
async def create_organization_contest(
        organization_contest: OrganizationContest,
...
async def get_contest_list_page_by_organization_id(
        organization_id: int,
        page: int,
        size: int,
        db: AsyncSession):
...
async def find_organization_contest_by_contest_id(
        contest_id, db):
    stmt = (
        select(OrganizationContest)
...
async def find_by_contest_and_user(
        contest_id: int,
        user_id: int,
        db: AsyncSession) -> Optional[ContestUser]:
    stmt = (
        select(ContestUser)
        .where(
...
async def find_announcement_by_announcement_id(
        announcement_id: int,
        db: AsyncSession):
    stmt = (
        select(ContestAnnouncement)
        .where(ContestAnnouncement.id == announcement_id)
    )
    result = await db.execute(stmt)
    return result.scalar_one_or_none()


async def delete_contest_announcement_by_announcement_id(
        announcement_id, db):
    stmt = (
...
async def find_contest_user_by_contest_id_and_user_id(
        contest_id: int,
        user_id: int,
        db: AsyncSession):
